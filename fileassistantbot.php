    ...
    public function onUpdateNewMessage($update)
    {
        ...
        try {
            if (isset($update['message']['media']) && ($update['message']['media']['_'] == 'messageMediaPhoto' || $update['message']['media']['_'] == 'messageMediaDocument')) {
                ...
                $sent_message = yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Generazione download link 0%', 'reply_to_msg_id' => $message_id]);
                $time = time();
                $last_progress = 0;
                ...
                $output_file_name = yield $this->download_to_dir($update, new \danog\MadelineProto\FileCallback(
                    ...
                    function ($progress) use ($update, $sent_message, &$last_progress, $queue_id) {
                        ...
                        if ($progress > $last_progress + 4) {
                            ...
                            try {
                                yield $this->messages->editMessage(['id' => $sent_message['id'], 'peer' => $update, 'message' => 'Generazione del download link... '.$progress.'%'], ['queue' => $queue_id]);
                            } catch (Exception $e) {
                            }
                        }
                    }
                ));
                yield $this->messages->editMessage(['id' => $sent_message['id'], 'peer' => $update, 'message' => 'Download link Generated in '.(time() - $time)." seconds!\n\n💾 ".basename($output_file_name)."\n\n📥 ".rtrim(WEBSERVER_BASE_URL, '/').'/'.str_replace(__DIR__.'/', '', str_replace(' ', '%20', $output_file_name))."\n\n Il link scadrà tra 24 ore.", 'reply_to_msg_id' => $message_id], ['queue' => $queue_id]);
            } elseif (isset($update['message']['message'])) {
                $message_id = $update['message']['id'];
                $text = $update['message']['message'];
                $chat_id = $update['message']['from_id'];
                if ($text == '/start') {
                    yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Ciao! Perfavore mandami il link del file che vuoi scaricare, per supportarmi entra in @GamingGroupIT Grazie!' => $message_id]);

                    return;
                }
                if ($text == '/speedtest') {
                    if (isset($this->latest_speedtest[$chat_id]) && time() - $this->l

atest_speedtest[$chat_id] < 3600) {
                        yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Puoi controllare la tua velocità di rete una volta ogni ora.', 'reply_to_msg_id' => $message_id]);

                        return;
                    }
                    $this->latest_speedtest[$chat_id] = time();
                    $sent_message = yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Test della tua velocità di rete...', 'reply_to_msg_id' => $message_id]);
                ...
                if ($url === false) {
                    yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Il formato dell url ė incorretto.'reply_to_msg_id' => $message_id]);
                ...
                if (empty($filename)) {
                    yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Can you check your URL? I\'Non sono in grado di rilevare il file da questo url.', 'reply_to_msg_id' => $message_id]);
                ...
                if (empty($headers['content-length'][0])) {
                    yield $this->messages->sendMessage(['peer' => $update, 'message' => 'Impossibile ottenere la dimensione del file.', 'reply_to_msg_id' => $message_id]);
                ...
                yield $this->messages->editMessage(['id' => $sent_message['id'], 'peer' => $update, 'message' => "📤La tua richiesta è in coda, non inviare altre richiete.\n🗂 File: ".$filename."\n🔗 URL: ".$url."\n💿 Dimensione del file: ".$this->formatBytes($filesize)."\n\n⌛ Avanzamento: 0%"]);
                $time = time();
                $last_progress = 0;
                $queue_id = $this->randomString().time();
...
'media' => [
                        ...
                        'file' => new \danog\MadelineProto\FileCallback($filepath, function ($progress) use ($update, $sent_message, &$last_progress, $filename, $filesize, $url, $queue_id) {
                            ...
                            if ($progress > $last_progress + 4) {
                                ...
                                try {
                                    yield $this->messages->editMessage(['peer' => $update, 'id' => $sent_message['id'], 'message' => "📤La tua richiesta è in coda, non inviare altre richieste. \n🗂 File: ".$filename."\n🔗 URL: ".$url."\n💿 Dimensione del file: ".$this->formatBytes($filesize)."\n\n⌛ Avanzamento: ".$progress.'%'], ['queue' => $queue_id]);
                                } catch (Exception $e) {
                                }
                            }
                        }),
